...
function valider(){
    ...
    conscienceUnchecked=!document.querySelector('input[name="ventilation"][value="spontanée"]').checked&!document.querySelector('input[name="ventilation"][value="mécanique"]').checked
    if(conscienceUnchecked){
        document.getElementById('récapitulatif').innerHTML="<p>Vous n'avez pas tout saisi<p>"
        return null
    }
    ...
    if(conscience=='conscient'){
        ...
        a=`<p>Nom : ${nom}</p><p>Prénom : ${prénom}</p><p>Date : ${date}</p>`
    }
    else{
        a=''
    }
